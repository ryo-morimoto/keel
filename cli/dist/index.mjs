...
var targets = {
  cc: {
    ...
    skillsDir: (scope, home, cwd) => scope === "user" ? path.join(home, ".claude", "skills") : path.join(cwd, ".claude", "skills"),
  ...
  codex: {
    ...
    skillsDir: (scope, home, cwd) => scope === "user" ? path.join(home, ".codex", "skills") : path.join(cwd, ".codex", "skills"),
    expandFields: (metadata) => {
      const fields = {};
      for (const [key, value] of Object.entries(metadata)) {
  ...
  opencode: {
    ...
    skillsDir: (scope, home, cwd) => scope === "user" ? path.join(home, ".config", "opencode", "skills") : path.join(cwd, ".opencode", "skills"),
    expandFields: (metadata) => {
      const fields = {};
      for (const [key, value] of Object.entries(metadata)) {
...
// src/skill-parser.ts
import { readFileSync, existsSync as existsSync2, readdirSync, cpSync, mkdirSync, writeFileSync } from "fs";
...
function parseSkillMd(skillMdPath) {
  ...
  const properties = jsYaml.load(frontmatterRaw);
  ...
  return {
    ...
    frontmatterRaw,
    body,
    sourceDir: path2.dirname(skillMdPath)
  };
}
...
function expandSkillForTarget(skill, target) {
  ...
  const frontmatter = jsYaml.dump(newProps, {
    lineWidth: -1,
    quotingType: '"',
    forceQuotes: false
  }).trimEnd();
...
var install_default = defineCommand({
  ...
  async run({ args }) {
    ...
    const pluginRoot = args.source ? path3.resolve(String(args.source)) : await resolvePluginSource();
    const skills = discoverSkills(pluginRoot);
    if (skills.length === 0) {
      console.error("No skills found in", pluginRoot);
      process.exit(1);
    }
    console.log(`Found ${skills.length} skill(s): ${skills.map((s2) => s2.name).join(", ")}`);
...
async function resolveTargets(targetName, home, cwd) {
  if (targetName === "all") {
    const detected2 = await detectInstalledTools(home, cwd);
    const active2 = detected2.filter((t2) => t2.detected);
    console.log(`Detected ${active2.length} agent(s):`);
    for (const tool of detected2) {
      console.log(`  ${tool.detected ? "✓" : "✗"} ${tool.displayName} — ${tool.reason}`);
    }
    return active2.map((t2) => t2.name);
  }
  if (targetName) {
  ...
  const active = detected.filter((t2) => t2.detected);
  if (active.length === 0)
    return [];
  if (active.length === 1) {
  ...
  return active.map((t2) => t2.name);
}
async function resolvePluginSource() {
  const scriptDir = new URL(".", import.meta.url).pathname;
  const repoRoot = path3.resolve(scriptDir, "..", "..");
  const { existsSync: existsSync3 } = await import("fs");
  if (existsSync3(path3.join(repoRoot, "skills", "keel", "SKILL.md"))) {
    const pluginRoot = path3.resolve(scriptDir, "..", "..", "..");
    if (existsSync3(path3.join(pluginRoot, ".claude-plugin", "plugin.json"))) {
      return pluginRoot;
    }
  }
  const { mkdtempSync } = await import("fs");
  const { execSync } = await import("child_process");
  const tmpDir = mkdtempSync(path3.join(os2.tmpdir(), "keel-install-"));
